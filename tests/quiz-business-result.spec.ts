...
test.describe('Sign-up quiz', () => {
  test(`A ${TEEN_FUNNEL.branch} visitor who finishes the quiz gets an account and a trial lesson`, async ({
    pageManager,
    apiManager,
  }) => {
    ...
    const run = await test.step('Answer every screen the funnel shows until it finishes', () => {
    ...
    await test.step(`Verify the trial outcome: ${run.outcome}`, async () => {
      if (run.outcome === 'trial-booked') {
        ...
        await expect
          .poll(async () => (lessons = await apiManager.allRight.getUpcomingTrialLessons()).length, {
            timeout: 20_000,
          })
          .toBeGreaterThan(0)
